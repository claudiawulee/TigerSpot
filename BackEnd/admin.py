...
def requests():
    ...
    if check is False:
        ...
        html_code = flask.render_template('Versus/challenges.html', challenges=pending_challenges, user=username_auth, users=flask.json.dumps(users), username=username)
...
def team():
    ...
    html_code = flask.render_template('team.html', username = username, top_player_username = top_player_username)
    response = flask.make_response(html_code)
    return response
...
def versus_func():
    ...
    if check is False:
        ...
        html_code = flask.render_template('Versus/challenges.html', users=flask.json.dumps(users), username=username)
...
def start_challenge(challenge_id=None, index=None):
    ...
    if index < len(versusList):
        ...
        if check is False:
            html_code = flask.render_template('contact_admin.html')
            return flask.make_response(html_code)
        html_code = flask.render_template('Versus/versusgame.html', challenge_id=challenge_id, index=index, link=link)
...
def submit2():
    ...
    if not currLat or not currLon:
        ...
        index = int(index) + 1
        html_code = flask.render_template('Versus/versusresults.html', dis = "No Submission", lat = None, lon = None, coor=coor, index=index, challenge_id=challenge_id, points=str(points), place=place)
        response = flask.make_response(html_code)
        return response
    ...
    if pic_status is False:
        ...
        points = "Already submitted."
    index = int(index) + 1
    html_code = flask.render_template('Versus/versusresults.html', dis = distance, lat = currLat, lon = currLon, coor=coor, index=index, challenge_id=challenge_id, points=str(points), place=place)
    response = flask.make_response(html_code)
    return response
...
def versus_stats():
    ...
    if check is False:
        ...
        html_code = flask.render_template('Versus/versus_stats.html', results=results, images=pictures)
